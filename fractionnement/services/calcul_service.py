...
def get_jours_hors_periode_principale(user: User, annee: int) -> int:
    ...
    # Calculer les jours fériés une seule fois pour toute l'année
    try:
        jours_feries = get_jours_feries(annee)
    except Exception as e:
        logger.error(f"Erreur lors du calcul des jours fériés pour l'année {annee}: {e}")
        jours_feries = set()  # Fallback: pas de jours fériés
    ...
    for periode in periodes:
        ...
        try:
            ...
            periode_entiere_hors = (
                (date_debut.month >= 11 and date_fin.month >= 11) or  # Nov-déc
                (date_debut.month <= 4 and date_fin.month <= 4) or    # Janv-avril
                (date_debut.month >= 11 and date_fin.month <= 4)       # Chevauche année
